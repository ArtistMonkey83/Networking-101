...
long chunk = strtol(argv[1], NULL, 10);
 
char re[chunk];
 
int reLen = chunk, bytes_recv = 0, h1_recv = 0, temp = 0;
 
char *header = "<h1>";
 
while(1)
{ 
//Continuously receive bytes
 
memset(re, 'a', chunk);//what if memset()???
 
temp = bytes_recv; 
//Initialize our bytes recieved counter
 
int new_bytes = recv(s, re, reLen, 0); //Increment our bytes recieved counter
 
if(new_bytes < chunk)//if there are fewer bytes recieved than expected
 
{
 
re[new_bytes] = '\0'; //end the c-string at the location after the last recieved byte
...
if(bytes_recv == temp)
{break;} 
//Evaluates to true if we tried to recieve more bytes and didn't
